...
public class BudgetSplitterApp {
...
public static void makeSplit(double totalAmount, ArrayList<Split> splitList){
    ...
    for(Split split : splitList){
        if(split.amount>amount){
            ...
            System.out.println(split.userName+" needs to pay back "+ (amount- split.amount));
        }
    ...
    public static ArrayList<Split> calculateSplitByUser(ArrayList<Expense> expenseList) {
        ...
        for(Expense expense : expenseList){
            ...
            if(split!=null){
                split.amount+=expense.amount;//add expenses to existing expense list to exact user
            }else{
                Split willBeAdded=new Split();//created new object
